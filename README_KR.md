...
`agents@0.20.1`과 `@cloudflare/ai-chat@0.10.1` 기준으로 검증했습니다. peer 범위는 의도적으로 넓게 잡았지만 엔드 투 엔드로 검증된 버전은 0.20.1뿐입니다. `agents` 0.7.0 -> 0.20.1 구간(마이너 14개)에서 선언된 이벤트 타입은 31개에서 91개로 늘었고 제거된 것은 없어 구형 마이너에서도 디코딩은 문제없을 것으로 보이나, 실제 검증 대상은 아닙니다. 이벤트 스키마는 최대한 상위 호환을 지향합니다. 알 수 없는 이벤트 타입이 와도 UI가 죽지 않고 스트림 탭에 raw JSON으로 표시됩니다.
...
상태 스냅샷은 collector가 손대는 유일한 페이로드이고, 나머지 이벤트 페이로드는 그대로 전달됩니다. 그중 하나는 대화 내용을 담고 있습니다. `chat:recovery:exhausted`에는 리커버리가 포기하기 직전까지 스트리밍된 어시스턴트 출력이 `partialText`와 `partialParts`로 실려 옵니다. 공개된 타입 선언에는 두 필드 모두 없습니다. `--record`로 세션을 기록하면 그 텍스트가 NDJSON 파일에 남습니다.
